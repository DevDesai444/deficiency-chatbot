...
- **RESOLVED (2026-08-14) — 06-06 live gate.** Was blocked on Databricks Nemotron serving entitlement. Resolved by β-pivot: the verifier runs on the already-served on-prem fleet (Llama 3.3 70B + 2 Qwen MoE) instead of Nemotron; the D-06 gate now runs live and passes conformance across all 3 models. Nemotron deploy remains deferred (notebook committed; restorable via `VERIFIER_MODEL_NAME` + the drafted support ticket if custom serving is ever enabled) but is no longer on the critical path.
- **CARRIED to Phase 7 — verifier precision.** Weak on-prem models don't discriminate reliably as SOLO single-shot verifiers (D-06b diagnostic; all fleet models keep-lean). Precision must come from Phase 7's architecture: full re-opened source+rule context (not truncated spans) + decorrelated Llama⟂Qwen multi-agent consensus. This is the central Phase 7 success criterion, proven by end-to-end F1 (zero-TP-loss vs P5).

- **[UPDATED 2026-08-14 — Phase 7 verifier: 5 production-path bugs FIXED; precision now bounded by 2 named levers, not hidden bugs.]** The live verify path was built entirely against offline scripted doubles and never run; five stacked bugs each made `downgraded=0` on the real corpus, now all fixed + regression-tested (tests/verify 21 passed, recall still zero-TP-loss): (1) `_reopen_full_source` anchor-unaware → added `_resolve_reopen_span` (structural_anchor.claim_span_id + absence sub_threshold_hits); (2) could_not_locate required BOTH halves → now gradeable if EITHER re-opens; (3) production `nt=None` → added `_reopen_full_nt` (doc NormalizedText from corpus cache); (4) `verify_once` offered no verdict tool + read `tool_calls` while models answered in content → added `submit_verdict` output tool (write-disabled invariant intact: it records a judgment, not a corpus write); (5) `_grounded_of` re-resolved against the narrow anchor span not the section shown → now grounds vs `source_text`. Verifier now genuinely discriminates live (Qwen122B DOWNGRADEs the planted MIN-FP with a grounding span). Commits adf9683, (verify_once), (grounding). **REMAINING precision levers (reviewer decision — recall/precision tradeoff):** (A) **Qwen3-next-80b returns unreadable→KEEP** on the verify path (flaky reasoning-endpoint tool emission) → 1 of 3 votes always lost; fix its tool-call reliability (same class as the Phase-6 Qwen budget/reasoning-content fix). (B) **consensus requires a grounded MAJORITY (≥2 of 3)** to downgrade (recall-safe by design); with Llama keep-leaning + Qwen3-next unreadable, at most Qwen122B (1 vote) fires → never majority → downgraded=0. Options: fix (A) so 2 Qwens can agree, and/or lower the threshold to "≥1 grounded downgrade among READABLE panel members" (more FP pruning, but a wrongly-downgraded real deficiency then drops from ACTIVE findings into coverage — a recall-in-the-surfaced-report cost the reviewer must weigh; downgrade-never-drop still holds so nothing is deleted). Also: the ~102 absence FPs need search-based verification (re-run the negative via search_corpus) to be disprovable — the re-open+judge model can only prune structural/reference FPs. Measured effect so far (tail off, real corpus): precision 0.0488→0.0504, recall 0.1875 held, ZERO TP lost. **UPDATE: lever A DONE (verifier offers only submit_verdict; all 3 fleet models now emit readable grounded verdicts). Lever B DONE as a reviewer-owned knob `VERIFIER_CONSENSUS_MODE` (default majority = recall-safe, unchanged; `any` = ≥1 grounded downgrade).** With `VERIFIER_CONSENSUS_MODE=any` the FULL chain works end-to-end live: 5 grounded downgrades on mvr1381, precision 0.0488→0.0522, F1 0.0774→0.0816, recall 0.1875 held, **ZERO TP lost** (all 6 GT retained — lowering the threshold was recall-safe in practice). Gain is modest because only STRUCTURAL/REFERENCE FPs are disprovable by re-open+judge; the ~102 ABSENCE FPs (the bulk) need **lever C = absence-search-verification** (give the write-disabled verifier `search_corpus` to RE-RUN THE NEGATIVE — confirm the required item is genuinely absent vs. actually present-but-missed-by-retrieval; the CoverageAbsenceAnchor.sub_threshold_hits already exist for this). Lever C is the remaining big precision unlock. Commits: (submit_verdict), (grounding), (lever A only-submit_verdict), (lever B VERIFIER_CONSENSUS_MODE).
- **[SUPERSEDED by the entry above — original finding] Verifier does not re-open ABSENCE/STRUCTURAL-anchored candidates → precision NOT improved on real corpus.** Phase 7 code is complete + offline-green (tests/verify 20 passed) + RECALL proven on the real corpus (beta-recall-gate PASS: 0.1875 held, 6 TPs, ZERO TP lost with the full verify layer present). BUT the first live exercise of `verify.driver.verify_and_assemble` on the real corpus revealed: `orchestrator._reopen_source` only handles `submission_span_id`, while real Phase-5 candidates — especially the ~102 absence over-emit (the bulk of the FP load) — have `submission_span_id=null` + an `absence_anchor`/`structural_anchor` instead. Every real candidate → `could_not_locate` → kept ACTIVE, never sent to the fleet → **downgraded=0, no FP pruning, precision unchanged.** The offline tests passed because their fixtures carried resolvable submission spans (not representative of the absence-heavy real mix). `grounding.py` is anchor-type-aware; the orchestrator's re-open GATE is not. **Fix (reviewer-guided design): give the verifier an absence/structural verification mode (re-open the RULE via rule_span_id + assess genuine absence from absence_anchor requirement_id/threshold/sub_threshold_hits, rather than requiring a source span), and make the tests/verify fixtures represent REAL Phase-5 candidate shapes so this class of bug is caught offline.** Then re-run the live verify-f1 precision proof (needs a legitimate Phase-5 baseline recapture — the authored-* baseline correctly HARD-FAILS a live run until a scorer-produced Phase-5 report is captured; the golden mvr1381_run3.json scores at the OLD Phase-3 0.071, so the reviewer must resolve the correct Phase-5 report source).
...
- [Phase ?]: 07-03: verifier orchestrator returns (faults, CoverageReport); DOWNGRADE lowers confidence never drops; Gap-1 assemble filter surfaces only KEEP-tier to the frozen F1 path while retaining DOWNGRADEd faults in coverage
