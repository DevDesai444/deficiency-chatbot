...
- **D-17:** **`parsed_partial` is a distinct, load-bearing status** (not `parsed`). A document that parses but **degrades** — scanned pages OCR'd to flat text, tables not reconstructed, pages skipped — is recorded as `parsed_partial` **with what specifically degraded**, never silently passed as fully `parsed`. This **mirrors Claude Code's `isPartialView`**: content the model has only partially seen must be marked so **downstream grounding refuses to treat it as complete evidence**. **Latent seam (NOT a live defect — corrected 2026-07-30):** `src/parse/ocr.py:122` is an *old-endpoint compatibility branch*. When it fires, the parser returns `payload, [], [], []` and `pdf.py:222` leaves `source="rapidocr"` — so the page record **claims OCR succeeded while blocks and tables are empty**. The currently deployed endpoint (`defpredict.main.rapidocr` v2, `DEPLOYMENT_READY`) returns `json.dumps(regions)` — a JSON string of `{text,x0,y0,x1,y1}` boxes that `_reconstruct_prediction` parses correctly via its `startswith("[")` path — so scanned-table extraction **works today**. An earlier claim in this document that the endpoint was degraded was wrong; **no redeploy task is needed**, and RapidOCR stays Databricks-only (no local fallback — user decision). The branch still matters as **regression insurance**: a rollback or version drift would silently lose scanned tables while reporting success. Preferred guard: mark that branch `source="rapidocr-flat-text"` so `parsed_partial` reads a *fact* rather than inferring degradation from empty blocks (which also matches a genuinely blank scanned page).
