...
- **D-STR6 — Rule span is OPTIONAL for the structural leg (nullable), resolved 2026-08-06 post-research.** Pure internal-consistency findings (aggregate recompute, summary-vs-detail mismatch) violate no specific FDA/ICH rule and emit with **no rule span**; only result-exceeds-spec-limit cites the limit (from the rulebook rule span when available). The emit gate (`emit_finding`, which currently hard-requires `rule_span_id`) must be relaxed to permit `null` ruleId for the structural leg — consistent with D-ENV1 ("rule span attached only when a rule applies", ruleId nullable). Do **not** invent a general "data-consistency" rule to force-fit a citation.
...
- **D-GRD4 — The synthetic fixture is MULTI-doc and doubles as the X1/X2 end-to-end scenario (resolved 2026-08-06 post-research).** Phase 0's current eval corpora (mvr1381/minispec) are single-document, so there is no true cross-document scenario to satisfy SC2. The committed synthetic fixture is therefore a **multi-document submission** (doc_a = QOS analog, doc_b = Module 3.2 analog) with a planted **X1** (QOS-vs-Module spec mismatch) and **X2** (cross-doc value contradiction), serving double duty: anti-overfitting guard corpus AND the cross-document end-to-end catch for SC2. It still honours all four D-GRD1 constraints (planted blind to the check code; different surface forms than mvr1381; realistic bge-m3 cosine regime; two-tier tripwire + real spec32s41 slow-lane witness).
