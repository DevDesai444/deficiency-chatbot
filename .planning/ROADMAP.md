...
- [ ] **Phase 0: Eval Harness** - Multi-doc ground-truth + per-stage metrics (recall-by-family); the continuous gate on every later phase
...
**Depends on**: Phase 1
**Requirements**: TOOLS-01, TOOLS-02, RULES-01, RULES-02, RULES-03, RULES-04
**Success Criteria** (what must be TRUE):
...
**Success Criteria** (what must be TRUE):
  ...
  3. Hard per-run token/step/wall-clock budgets and a **circuit breaker** are enforced **in code, not prompt** — a runaway load test halts at the ceiling and returns the grounded partial, never crashing or overspending.
...
|-------|----------------|--------|-----------|
| 0. Eval Harness | 1/4 | In Progress|  |
