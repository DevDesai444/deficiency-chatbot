...
**Outcome**: **COMPLETE** (2026-08-06). 3/3 plans; verification **passed** (4/4 must-haves). `absence_of_evidence` recovered `0.000 → 1.000` via a deterministic pre-loop pass (β direction, not a model loop). Code review found + fixed one recall blocker (CR-01: whole-section pass now edge-driven, not raw `.family`) and hardened the anti-overfitting guard (WR-01/02: structural NO-CONSTANT + coverage/absence-gates wired into CI + corpus-gated slow lane). Held-out generality witness (spec32s41) independently run and passed. **Phase-5 handoff**: absence threshold `0.04` sits above the RRF score ceiling (~0.0328) → retrieval leg non-discriminative (sanctioned D-ABS2 over-emit); RECALL-05 must give the threshold real dynamic range, Phase 7 inherits the pruning load.
...
**Phase 5 outcome (Complete 2026-08-08, verified 5/5):** deterministic recall engine complete — absence 1.000; structural (SUM/MAX aggregate) + cross-doc reference graph (X1 Compound-B 0.18 > NMT 0.15, hard-caught end-to-end) + precedent legs live; all rulebook/structure/graph-general behind the enforced anti-overfitting guard (behavior-transfer PRIMARY + NO-CONSTANT); frozen measurement preserved byte-identical; zero true positives lost. Full suite 589 passed / 0 failed. **Correction (owned by 5R):** SC5's *real-corpus* recall clause was NOT met at phase close — the engine scored **0.000 on the real corpus** because emitted findings carried empty `evidence` and the scoring relabel/cache bridge suppressed the structural leg (delivery failures, not detection failures). This is now remediated and measured by **Phase 5R**.

**Phase 5R — real-corpus recall bridge (Complete 2026-08-08):** R1 populated `evidence` (verbatim span text) on all three legs; R2 fixed the relabel/cache bridge so structural survives scoring; R3 re-measured on the real corpus (frozen path): **aggregate recall 0.1875 (6/32)** — mvr1381 0.179 (A-09, A-11, B-01, **C-01**, C-06), minispec 0.250 (MS-01) — **> 0.107 golden and > 0.071 agent baselines; C-01 matched for the first time in project history** (up from 0.000). R5 added the additive `beta-recall-gate` ratchet (matched_set + 0.1875 floor, CI corpus-gated lane). **Named residual gaps (Phase-7 handoff):** B-08 (absence localization does not yet quote anchor-bearing submission text), C-02 (mvr 0.15% spec-exceedance — prose-located limit, honestly deferred per D-STR5/D-STR6), MS-02 (word-anchor "Maximum" vs value-quoting evidence — a possible future general improvement is quoting the claim-row label cell; not ordered). ~102-fp over-emit is the Phase-7 verifier's `dedup_key` pruning load. Golden `found_set` unchanged (no swap — it would shrink protection).
...
- [x] 06-04-PLAN.md — client.py allow-list guard + extra_body passthrough + registry field-level hints (MODEL-01, RELIABILITY-01/02)
- [x] 06-05-PLAN.md — Nemotron deployment notebook + MODEL-02 pre-wiring probes (MODEL-01, MODEL-02) *(notebook built + committed; live deploy DEFERRED — Nemotron unservable on aip-amn-dev)*
- [x] 06-06-PLAN.md — D-06 probe suite gate + phase completion (MODEL-01/02, RELIABILITY-01/02/03) *(β-pivot: verifier runs on served fleet Llama 3.3 70B + 2 Qwen MoE instead of blocked Nemotron; D-06a conformance PASSES all 3 models × 2 modes; D-06b discrimination reclassified to non-blocking diagnostic, precision proven at Phase 7)*

> **β-PIVOT (2026-08-13, reviewer-approved):** Nemotron-Super-49B is unservable on aip-amn-dev (custom-entrypoint serving disabled workspace-wide; H100 tiers unenrolled). The verifier role runs on the already-served on-prem fleet — **Llama 3.3 70B + Qwen35-122b-a10b + Qwen3-next-80b-a3b** (config.VERIFIER_FLEET, env-overridable; Nemotron restorable via `VERIFIER_MODEL_NAME`). Recall-safe by construction (recall deterministic in P5; verifier downgrade-never-drop in P7 → model strength is precision-only). Three Llama-on-Databricks tool-reliability failure modes surfaced and were fixed generally (invariant-aligned prompt, empty-`grounding_span` corrective retry, native-`<function=>` 400 recovery), plus reasoning-model token budgets + family-aware `/think`·`/no_think`. On-prem law upheld. **D-06b single-shot discrimination reclassified to non-blocking diagnostic** (weak-model discrimination is unstable solo, and the 2-FP/115 set trips the 0.95 tripwire even for a perfect verifier); **precision is proven at Phase-7 F1** with full re-opened context + decorrelated Llama⟂Qwen consensus.
