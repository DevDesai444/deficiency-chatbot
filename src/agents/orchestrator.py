...
def run_pipeline(pdf_path: str, job_id: str) -> RecommendationSet:
    ...
    intermediate_report = run_extraction(groups=groups, document_name=doc["filename"], document_type="", job_id=job_id)

    update_job_status(job_id, "detecting", intermediate_report=intermediate_report.model_dump())

    flaw_report = run_flaw_detection(intermediate_report=intermediate_report, document_section=CTDSection.UNKNOWN, job_id=job_id,)
    
    
    update_job_status( job_id, "correnting", flaw_report = flaw_report.model_dump())
    
    result = run_correction_loop( flaw_report=flaw_report, job_id=job_id,)
